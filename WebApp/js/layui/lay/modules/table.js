...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  Class.prototype.fullSize = function(){
    ...
    //减去列头区域的高度
    bodyHeight = parseFloat(height) - (that.layHeader.is(":visible") ? that.layHeader.outerHeight() : 38); //此处的数字常量是为了防止容器处在隐藏区域无法获得高度的问题，暂时只对默认尺寸的表格做支持。
    
    //减去工具栏的高度
    if(options.toolbar){
        bodyHeight = bodyHeight - (that.layTool.is(":visible") ? that.layTool.outerHeight() : 50);
    }
    
    //减去统计朗的高度
    if(options.totalRow){
        bodyHeight = bodyHeight - (that.layTotal.is(":visible") ? that.layTotal.outerHeight() : 40);
    }

    //减去分页栏的高度
    if(options.page){
        bodyHeight = bodyHeight - (that.layPage.is(":visible") ? that.layPage.outerHeight() : 41) - 2;
    }
